...
## Installation
The `itnb`-package depends on `R` (>= 4.1), `Rcpp`, `RcppArmadillo`, `roptim`, and `stats`. As the package is not available on CRAN, devtools is needed to install the package from github. From `R`, run the following commands:  
```r
